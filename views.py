...
def check_avail(doc, dt, tm):  # Check if doctor is available in a given slot
    tm = tm.strip()  # Remove leading/trailing spaces
    print(f"Raw time: {tm}")
    
    if not tm:  # If `tm` is empty, return False
        print("Error: Time is empty.")
        return False
    
    try:
        # Initialize AM/PM as 'AM' by default
        ampm = 'AM'

        # Check if the time string has AM/PM
        if 'AM' in tm or 'PM' in tm:
            time_part, ampm = tm.split()  # Split into time and AM/PM
        else:
            time_part = tm  # If no AM/PM, assume it's in 24-hour format

        hr, mn = time_part.split(':')  # Split time into hour and minute
        print(f"Hours: {hr}, Minutes: {mn}, AM/PM: {ampm}")

        # Convert hour and minute to integers
        hr = int(hr)
        mn = int(mn)
        
        # Convert to 24-hour time format if it's AM/PM
        if ampm == 'PM' and hr != 12:
            hr += 12  # Convert PM hours (except 12 PM) to 24-hour format
        elif ampm == 'AM' and hr == 12:
            hr = 0  # 12 AM is midnight, which is 0 hours in 24-hour format
        
        print(f"Converted hour: {hr}, Converted minute: {mn}")
        
    except ValueError:
        print("Error: Invalid hour or minute value.")
        return False
    except Exception as e:
        print(f"Unexpected error: {e}")
        return False

    # Create a time object for the given time slot
    ftm = time(hr, mn, 0)  # Create a time object for the given slot
    print(f"Converted time object: {ftm}")
    
    # Check if the time is within valid hours (9:00 AM - 5:00 PM excluding 1:00 PM - 2:00 PM)
    if ftm < time(9, 0, 0) or ftm > time(17, 0, 0):
        print(f"Error: Time {ftm} is outside the working hours (9 AM - 5 PM).")
        return False  # Slot is outside working hours, reject booking
    
    if time(13, 0, 0) <= ftm < time(14, 0, 0):
        print(f"Error: Time {ftm} is during lunch hours (1 PM - 2 PM).")
        return False  # Slot is during lunch hours, reject booking

    # Define the session time range (for example, 9 AM - 11 AM)
    session_start = time(9, 0, 0)  # Start of session (9 AM)
    session_end = time(11, 0, 0)   # End of session (11 AM)
    
    # Check if the selected time is within the session range (9 AM - 11 AM)
    if session_start <= ftm < session_end:
        # Count how many appointments are already booked in this session (9 AM - 11 AM)
        appointments_in_session = Appointment.objects.filter(
            status=True, doctor=doc, calldate=dt, 
            calltime__gte=session_start, calltime__lt=session_end
        ).count()

        print(f"Appointments in this session (9-11 AM): {appointments_in_session}")
        
        # If there are more than 100 appointments already in the session, reject the booking
        if appointments_in_session >= 100:
            print(f"Error: Session from 9 AM to 11 AM is fully booked (100 slots already filled).")
            return False  # Slot is full, reject booking

    # If the code reaches here, we can safely proceed to check the individual time slot:
    appointments = Appointment.objects.filter(status=True, doctor=doc, calldate=dt)
    for appointment in appointments:
        if ftm == appointment.calltime:
            print(f"Error: Slot {ftm} is already booked by another patient.")
            return False  # Slot is taken, reject booking

    print(f"Slot {ftm} is available.")
